...
const FILE_PATH = './src/components/SectionDivider.jsx';
let content = fs.readFileSync(FILE_PATH, 'utf8');

content = content.replace(/\{\/\* Left line \*\/\}[\s\S]+?\{\/\* Icon wrapper with premium glassmorphism & glow \*\/\}/, `      {/* A single solid line across the divider */}
      <div
        style={{
          position: "absolute",
          top: "50%",
...
content = content.replace(/\{\/\* Right dot \*\/\}[\s\S]+?<\/div>\s*<\/div>/, `</div>
    </div>`);

content = content.replace(/export const IconLedger = \([\s\S]+?\);/g, `export const IconLedger = (
...
content = content.replace(/export const IconVideo = \([\s\S]+?\);/g, `export const IconVideo = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/>
  </svg>
);`);

content = content.replace(/export const IconGrid = \([\s\S]+?\);/g, `export const IconGrid = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
    <path d="M3 3v8h8V3H3zm10 0v8h8V3h-8zM3 13v8h8v-8H3zm10 0v8h8v-8h-8z"/>
  </svg>
);`);

content = content.replace(/export const IconRocket = \([\s\S]+?\);/g, `export const IconRocket = (
...
content = content.replace(/export const IconCloud = \([\s\S]+?\);/g, `export const IconCloud = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
    <path d="M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96z"/>
  </svg>
);`);

content = content.replace(/export const IconShield = \([\s\S]+?\);/g, `export const IconShield = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm-2 16l-4-4 1.41-1.41L10 14.17l6.59-6.59L18 9l-8 8z"/>
  </svg>
);`);

content = content.replace(/export const IconChart = \([\s\S]+?\);/g, `export const IconChart = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
    <path d="M5 9.2h3V19H5zM10.6 5h2.8v14h-2.8zm5.6 8H19v6h-2.8z"/>
  </svg>
);`);

content = content.replace(/export const IconUtensils = \([\s\S]+?\);/g, `export const IconUtensils = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
    <path d="M11 9H9V2H7v7H5V2H3v7c0 2.12 1.66 3.84 3.75 3.97V22h2.5v-9.03C11.34 12.84 13 11.12 13 9V2h-2v7zm5-3v8h2.5v8H21V2c-2.76 0-5 2.24-5 4z"/>
  </svg>
);`);

content = content.replace(/export const IconLink = \([\s\S]+?\);/g, `export const IconLink = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
    <path d="M3.9 12c0-1.71 1.39-3.1 3.1-3.1h4V7H7c-2.76 0-5 2.24-5 5s2.24 5 5 5h4v-1.9H7c-1.71 0-3.1-1.39-3.1-3.1zM8 13h8v-2H8v2zm9-6h-4v1.9h4c1.71 0 3.1 1.39 3.1 3.1s-1.39 3.1-3.1 3.1h-4V17h4c2.76 0 5-2.24 5-5s-2.24-5-5-5z"/>
  </svg>
);`);

content = content.replace(/export const IconHandshake = \([\s\S]+?\);/g, `export const IconHandshake = (
  <svg width="28" height="28" viewBox="0 0 256 256" fill="currentColor">
...
content = content.replace(/export const IconLayers = \([\s\S]+?\);/g, `export const IconLayers = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
...
content = content.replace(/export const IconClipboard = \([\s\S]+?\);/g, `export const IconClipboard = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
    <path d="M19 3h-4.18C14.4 1.84 13.3 1 12 1c-1.3 0-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm2 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/>
  </svg>
);`);

content = content.replace(/export const IconStar = \([\s\S]+?\);/g, `export const IconStar = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
  </svg>
);`);

content = content.replace(/export const IconSettings = \([\s\S]+?\);/g, `export const IconSettings = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
...
content = content.replace(/export const IconDollar = \([\s\S]+?\);/g, `export const IconDollar = (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
